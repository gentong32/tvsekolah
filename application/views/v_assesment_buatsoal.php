...
<div class="container" style="color:black;margin-top: 60px; max-width: 900px">
	<center><span style="font-size:20px;font-weight:Bold;">ASSESMENT <?php echo $tasses[substr($asseske,0,1)]." - ".substr($asseske,1,1);?><br></span></center>
	<!---->
	...
	<div id="addeditsoal" style="display: none">
		<div class="row">
			...
			echo form_open('seting/updatesoal/'.$asseske, $attributes);
			?>
	...
	<div id="dmenubawah" style="margin-left:25px;margin-bottom: 30px;">
		<button class="btn btn-primary" onclick="return kembali();">Kembali</button>
...
<script>
	...
	$(document).ready(function () {
		...
		function uploadgbr(idx, field, data) {
			$('#loading' + idx).show();
			$.ajax({
				url: '<?php echo base_url(); ?>seting/upload_gambarsoal/<?php echo $asseske;?>/' + pilihan + '/' + idx + '/' + field,
				type: "post",
				data: data,
				processData: false,
				contentType: false,
	...
	function ditambah() {
		...
		$.ajax({
			...
			dataType: 'json',
			cache: false,
			url: '<?php echo base_url();?>seting/insertsoal/<?php echo $asseske;?>',
	...
	function dibatal() {
		...
		if (soalbaru == 1) {
			$.ajax({
				url: "<?php echo base_url();?>seting/delsoal",
				method: "POST",
	...
	function diedit(nomor) {
		...
		if (soalgbr[nomor] == "") {
			...
			$("#previewinggbsoal").attr("src", "<?php echo base_url();?>uploads/assesment/" + soalgbr[nomor]);
		...
		if (opsiagbr[nomor] == "") {
			...
			$("#previewinggbopsia").attr("src", "<?php echo base_url();?>uploads/assesment/" + opsiagbr[nomor]);
		...
		if (opsibgbr[nomor] == "") {
			...
			$("#previewinggbopsib").attr("src", "<?php echo base_url();?>uploads/assesment/" + opsibgbr[nomor]);
		...
		if (opsicgbr[nomor] == "") {
			...
			$("#previewinggbopsic").attr("src", "<?php echo base_url();?>uploads/assesment/" + opsicgbr[nomor]);
		...
		if (opsidgbr[nomor] == "") {
			...
			$("#previewinggbopsid").attr("src", "<?php echo base_url();?>uploads/assesment/" + opsidgbr[nomor]);
		...
		if (opsiegbr[nomor] == "") {
			...
			$("#previewinggbopsie").attr("src", "<?php echo base_url();?>uploads/assesment/" + opsiegbr[nomor]);
	...
	{
		if (jmlsoal>0)
		window.open("<?php echo base_url();?>seting/soal/tampilkan/<?php echo $asseske;?>","_self");
		else
	...
	function kembali()
	{
		window.open("<?php echo base_url();?>seting/soal","_self");
	}
