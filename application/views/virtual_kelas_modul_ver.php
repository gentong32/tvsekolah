...
foreach ($dafpaket as $datane) {
	if (in_array($datane->id_mapel, $mapelgakdobel)) {

	} else {
		$mapelgakdobel[] = $datane->id_mapel;
	}
	if ($datane->id_mapel == 0)
		$adamapelkosong++;
	...
	if ($datane->nama_paket == "UTS" || $datane->nama_paket == "UAS" || $datane->nama_paket == "REMEDIAL UTS" || $datane->nama_paket == "REMEDIAL UAS") {
		...
		if ($tglvicon == "2021-01-01 00:00:00")
			...
			$tayang_paket[$jml_paket] = substr($datane->tglvicon, 8, 2) . ' ' . $namabulan[(int)(substr($datane->tglvicon, 5, 2))] . ' ' . substr($datane->tglvicon, 0, 4) . ' - ' . substr($datane->tglvicon, 11, 5) . ' WIB';
...
$tjmlmapel = sizeof($mapelgakdobel);
if ($adamapelkosong > 0)
	$tjmlmapel--;
			...
			<div class="row">
				...
				<center>
					<div style="font-size: 18px;">
						<?php
						if ($tjmlmapel < 4)
							echo "Minimal harus tersedia 4 mapel yang masing-masing terdiri atas 4 modul untuk setiap bulannya.";
						else
							echo "Terdapat $tjmlmapel mapel tersedia." ?>
						<?php
						if ($adamapelkosong > 0)
							echo "<br>Ada " . $adamapelkosong . " modul tanpa mapel"; ?>
					</div>
				</center>
				...
				<div style="margin-bottom: 10px;">
					...
					<button class="btn-main"
							onclick="window.location.href='<?php echo base_url(); ?>virtualkelas/modul_guru/verifikator'">
						Tampilkan Modul
					</button>
				</div>
				<hr style="margin-bottom:5px;">
			<div style="color:red; margin-bottom:20px;"><i>Silakan menentukan jadwal UTS/UAS</i></div>
			</div>
