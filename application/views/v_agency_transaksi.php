...
if ($tglsekarang < 20) {
	...
	// "MASIH DIBAWAH TANGGAL 20, JADI HANYA BISA AMBIL TRANSAKSI SEBELUM TGL: " . $tglbatastarik;
} else {
	$tglbatastarik = $batasbulankemarin->format('d-m-Y');
...
foreach ($daftransaksi as $datane) {
	...
	if ($datane->status_ag == 1 && $datane->siapambil==2) {
		$jmltarik = $jmltarik + $bersih;
	} else if ($datane->status_ag == 2) {
		...
		<div class="container">
			<div class="row">
				...
				<center><span style="font-size:18px;font-weight: bold;">TABEL TRANSAKSI KELAS VIRTUAL<br><br></span>
					<button onclick="window.open('<?php echo base_url(); ?>agency/transaksi_sekolah','_self');">
...
<script>
	...
	function tarikdana() {
		...
		?>
		$.ajax({
			url: "<?php echo base_url();?>payout/request_bank_transaction_ag/",
			method: "GET",
			data: {},
			success: function (result) {
				if (result == "ok")
