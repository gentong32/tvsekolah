...
if ($asal == "owner")
	$alamat = base_url() . "channel/soal/buat/" . $linklist;
else
	$alamat = base_url() . "channel/guru/chusr" . $kd_user . "/" . $linklist;
						...
						<div class="LayoutGrid1">
							<div class="row">
								<div class="col-1" style="text-align:right;margin-bottom: 15px;">
									<button onclick="selesai()" style="visibility: hidden;" id="tbselesai">Selesai
									</button>
...
<script>
	...
	var hitungjawab = 0;
	var nilaiakhir = 0;
	<?php if ($judul=="UTS" || $judul == "UAS" || $judul == "REMEDIAL UTS" || $judul == "REMEDIAL UAS"){ ?>
	var bataswaktu = new Date("<?php echo $tglselesaiujian;?>");
	<?php } ?>
	var waktuskr = new Date();
	...
	function panggilsoal() {
		...
		isihtmlsoal = "<div style='margin-top:20px;' class='wb_LayoutGrid1'>" + " \r\n" +
			...
			"<div class='col-1'>" + " \r\n" +
			<?php if ($asal == "owner") {?>
			"<button onclick='kembalichannel()' id='tbulangi'>KEMBALI</button> \n" +
			<?php } ?>
			"<center>\n" +
			"\t\t\t\t\t\t\t<div style=\"font-size: 14px;\">\n" +
			"\t\t\t\t\t\t\t\tSoal Latihan<br>\n" +
			...
			"\t\t data-year=\"<?php echo substr($tglselesaiujian, 0, 4); ?>\"\n" +
			"\t\t data-month=\"<?php echo substr($tglselesaiujian, 5, 2); ?>\"\n" +
			"\t\t data-day=\"<?php echo substr($tglselesaiujian, 8, 2); ?>\"\n" +
			"\t\t data-hour=\"<?php echo substr($tglselesaiujian, 11, 2); ?>\"\n" +
			"\t\t data-minute=\"<?php echo substr($tglselesaiujian, 14, 2); ?>\"\n" +
			"\t\t data-second=\"<?php echo substr($tglselesaiujian, 17, 2); ?>\"></div>\n" +
			"</div>"+
			<?php } ?>
			"\t\t\t\t\t\t\t<hr>\n" +
			"\t\t\t\t\t\t</center>" +
	...
	<?php if ($judul=="UTS" || $judul == "UAS" || $judul == "REMEDIAL UTS" || $judul == "REMEDIAL UAS"){ ?>
	iwaktu = setInterval(cekwaktu, 1000);
	function cekwaktu()
	{
		if( new Date() >= bataswaktu){
			selesai();
			clearInterval(iwaktu);
		}
	}
	<?php } ?>
