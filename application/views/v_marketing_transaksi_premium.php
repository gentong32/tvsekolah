...
foreach ($daftransaksi as $datane) {
	$kotor = floor($datane->fee_siambruto);
	$pph = ceil($kotor * $persenfeeam / 100);
	$bersih = $kotor - $pph;
	$jmldana = $jmldana + $bersih;
	if ($datane->status_siam == 1 && $datane->siapambil==2) {
		$jmltarik = $jmltarik + $bersih;
	} else if ($datane->status_siam == 2) {
		...
		<div class="container">
			<div class="row">
				...
				<center><span style="font-size:18px;font-weight: bold;">TABEL TRANSAKSI IURAN SEKOLAH<br><br></span>
					<button onclick="window.open('<?php echo base_url(); ?>marketing/transaksi_kelasvirtual','_self');">Transaksi
						Kelas Virtual
					</button>
				</center>
...
<script>
	...
	function tarikdana() {
		...
		?>
		$.ajax({
			url: "<?php echo base_url();?>payout/request_bank_transaction_premium/",
			method: "GET",
			data: {},
			success: function (result) {
				if (result == "ok")
