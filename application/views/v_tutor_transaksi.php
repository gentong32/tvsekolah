...
foreach ($daftransaksi as $datane) {
	...
	if ($datane->status_kontri == 1 && new DateTime($datane->tgl_beli) < $batasbulankemarin) {
		$jmltarik = $jmltarik + $bersih;
	} else if ($datane->status_kontri == 2) {
	...
	<section aria-label="section" class="pt30">
		<div class="container">
			<div class="row">
				<br>
				<center>
					<?php if ($this->session->userdata('bimbel') == 3 && $this->session->userdata('verifikator') != 3) { ?>
						<span style="font-size:16px;font-weight: bold;">TABEL TRANSAKSI TUTOR BIMBEL<br><br></span>
					<?php } else { ?>
						<span style="font-size:16px;font-weight: bold;">TABEL TRANSAKSI KELAS VIRTUAL<br><br></span>
					<?php } ?>
				</center>
				<hr>
...
<script>
	...
	$(document).ready(function () {
		...
		foreach ($daftransaksi as $datane) {
			...
			if ($datane->status_kontri == 1)
				$istatus = "-";
			else if ($datane->status_kontri == 2)
				$istatus = "Sudah ditransfer";
			else if ($datane->status_kontri == 3)
				$istatus = "Sudah ditransfer";
	...
	function tarikdana() {
		...
		?>
		$.ajax({
			url: "<?php echo base_url();?>payout/request_bank_transaction_kontri/",
			method: "GET",
			data: {},
			success: function (result) {
				if (result == "ok")
