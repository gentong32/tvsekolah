		...
		<div class="row">
			<div class="form-group" style="margin-top: 10px;">
				...
				<label for="inputDefault" class="col-md-12 control-label">
					<div id="soalnomor" style="margin-bottom:15px;text-align: center;font-weight: bold;font-size: larger">URAIAN</div>
				</label>
...
<script>
	...
	function updatemateri() {
		...
		$.ajax({
			url: "<?php echo base_url();?>seting/updateesay/<?php echo $untuksi;?>",
			method: "POST",
	...
	function lihattugas() {
		window.open("<?php echo base_url();?>seting/esay/tampilkan/<?php echo $untuksi;?>","_self");
	}

	function kosonginfile() {
		$.ajax({
			url: "<?php echo base_url();?>seting/kosonginfiletugas",
			method: "POST",
			data: {untuksi: "<?php echo $untuksi;?>"},
			success: function (result) {
				if (result == "sukses")
					document.getElementById('btn_file').style.display = 'none';
				else
	...
	$('#submitdok').submit(function (e) {
		e.preventDefault();
		$.ajax({
			url: '<?php echo base_url(); ?>seting/upload_doktugas/<?php echo $untuksi;?>',
			type: "post",
