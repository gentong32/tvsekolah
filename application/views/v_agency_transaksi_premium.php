...
foreach ($daftransaksi as $datane) {
	$kotor = floor($datane->fee_agency);
	$pph = ceil($datane->fee_agency*$pphagency/100);
	$bersih = $kotor - $pph;
	$jmldana = $jmldana + $bersih;
	//new DateTime($datane->tgl_bayar) < $batasbulankemarin
	if ($datane->status_agency == 1 && $datane->siapambil==2) {
		$jmltarik = $jmltarik + $bersih;
	} else if ($datane->status_agency == 2) {
		$jmlproses = $jmlproses + $bersih;
	}

	echo $datane->siapambil."<br>";
...
<div class="no-bottom no-top" id="content">
	...
	<section aria-label="section" class="pt30">
		<div class="container">
			<div class="row">
				<br>
				<center><span style="font-size:18px;font-weight: bold;">TABEL TRANSAKSI IURAN SEKOLAH<br><br></span>
					<button onclick="window.open('<?php echo base_url(); ?>agency/transaksi_virtualkelas','_self');">Transaksi Kelas
						Virtual
					</button>
				</center>
				...
				<div id="tabel1" style="font-size:1em;margin-left:1px;margin-right:1px;">
					<table id="tbl_user" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%">
						...
						<tr>
							...
							<th style="width:2%;text-align:center">Tanggal Bayar</th>
							<th style='width:20%;text-align:center'>Nama Sekolah</th>
							<th style='width:20%;text-align:center'>Jenis Iuran</th>
...
<script>
	...
	$(document).ready(function () {
		...
		foreach ($daftransaksi as $datane) {
			...
			$pph = ceil($datane->fee_agency*$pphagency/100);
			$bersih = $kotor - $pph;
			$kode = substr($datane->order_id,0,2);
			if ($kode=="TV")
				$jeniskelas = "Reguler";
			else if ($kode=="TP")
				$jeniskelas = "Pro";
			...
			if ($datane->status_agency == 1)
				$istatus = "-";
			else if ($datane->status_agency == 2)
				$istatus = "Sudah Ditransfer";

			echo "data.push([ " . $jml_user . ", \"" . namabulan_pendek($datane->tgl_bayar) .
				"\", \"" . $datane->nama_sekolah .
				"\", \"" . $jeniskelas .
	...
	function tarikdana() {
		...
		?>
		$.ajax({
			url: "<?php echo base_url();?>payout/request_bank_transaction_premium_ag/",
			method: "GET",
			data: {},
			success: function (result) {
				if (result == "ok")
