...
if (isset($sebagai))
	{
		if ($sebagai>0)
		{
			$userData['sebagai']=$sebagai;
			$this->session->set_userdata('sebagai', $sebagai);
		}
	}
...
//color: #be3f3f">' . $tgl1->diff($tgl2)->format("(Masa aktif verifikator, tersisa %a hari lagi)") . '</span><br>';
				...
				<ul class="cd-tabs__panels">
					...
					<li id="tab-inbox" class="cd-tabs__panel text-component <?php echo $selectedb1;?>">
						<legend>Data Personal Calon Verifikator
						</legend>
					...
					<li id="tab-new" class="cd-tabs__panel text-component <?php echo $selectedbkelas;?>">
						...
						if ($userData['sebagai'] == 1 || $userData['sebagai'] == 2) echo 'block';
							...
							<div class="form-group" id="ketsekolahbaru" style="display:none;">
								<div id="ketsekolah" style="margin-left:30px;font-weight:bold;color: #ff2222">Nomor kode salah atau belum terdaftar
								</div>
								<button style="margin-left: 30px;" class="myButtonblue"
										onclick="return tambahsekolah();">Ajukan Data Baru
								</button>
							</div>
							<div class="form-group" id="ketadaver" style="display:none;">
								<div id="ketver" style="margin-left:30px;font-weight:bold;color: #ff2222">Sudah terdaftar verifikator pada sekolah ini
								</div>
							</div>
...
<script>
		...
		$.ajax({
			...
			success: function (result) {
				...
				$.each(result, function (i, result) {
					...
					if (result.ada_ver == "ada") {
						document.getElementById('ketadaver').style.display = "block";
					}
					else{
						document.getElementById('ketadaver').style.display = "none";
					}
	...
	function cekupdate() {
		...
		if (document.getElementById('ketadaver').style.display == "block") {
			ijinlewat2 = false;
		}
