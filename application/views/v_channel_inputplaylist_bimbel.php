...
foreach ($dafvideo as $datane) {
	...
	$idjenis[$jml_video] = $datane->idjenis;
	$jenis[$jml_video] = $txt_jenis[$datane->idjenis];
...
if ($jml_video == 0) { ?>
			...
			<div class="container">
				<div class="row">
					<div>
						...
						<button class="btn-main"
								onclick="window.location.href='<?php echo base_url() . 'video/saya/bimbel/'; ?>'">
							Semua Video Saya
						</button>
					</div>
					<hr style="margin-top: 10px;">
					<center>
						<span style="font-size:16px;font-weight: bold;">VIDEO UNTUK BIMBEL BELUM TERSEDIA</span>
					</center>
					<br>
					<center>
			<span style="font-size:16px;font-weight: bold;">Silakan Tambahkan Video menggunakan tombol Semua Video Saya,<br>
				kemudian ubah Sifat dari Publik menjadi Bimbel terlebih dahulu.</span><br><br>
						<span style="color:red;font-size:16px;font-weight: bold;">Pastikan status Verifikator adalah Lulus (sudah diverifikasi Agency / Verifikator Bimbel).</span>
					</center>
				</div>
			</div>
		</section>
		...
		<section aria-label="section" class="pt30">
			<div class="container">
				<div class="row">
					<br>
					<center>
						<span style="font-size:18px;font-weight: bold;">Video untuk BIMBEL<br><?php echo $namapaket; ?></span>
						<br>
					...
					<div style="margin-top: 25px;margin-bottom: 10px;">
						<button class="btn-main" type="button" onclick="return kembali();">Kembali
						</button>
						<button class="btn-main"
								onclick="window.location.href='<?php echo base_url().'video/saya/bimbel/';?>'">Semua Video Saya
						</button>
						<button class="btn-main"
								onclick="window.open('<?php echo base_url()."bimbel/materi/buat/". $kode_paket . $tambahalamat; ?>', '_self')">Materi
						</button>
						<button class="btn-main"
							onclick="window.open('<?php echo base_url()."bimbel/soal/buat/". $kode_paket . $tambahalamat; ?>', '_self')">Soal Latihan
						</button>
						<button class="btn-main"
							onclick="window.open('<?php echo base_url()."bimbel/tugas/tampilkan/". $kode_paket . $tambahalamat; ?>', '_self')">Tugas
						</button>
					</div>
					...
					<div style="margin-bottom: 15px;">
						<button class="btn-main" type="button"
								onclick="window.location.href='<?php echo base_url(); ?>bimbel/urutanplaylist_bimbel/<?php echo $kodepaket; ?>'"
								class=""
	...
	<div style="margin:auto;width:auto;color:#000000;background-color:white;margin-bottom: 20px;">
		<center>
...
<script>
	...
	function masukinlist(idx) {
		...
		$.ajax({
			url: "<?php echo base_url();?>channel/masukinlist_bimbel",
			method: "POST",
