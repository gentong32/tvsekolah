...
<script>
	...
	function updatemateri() {
		...
		$.ajax({
			url: "<?php echo base_url();?>channel/updatemateri/<?php echo $linklist;?>",
			method: "POST",
	...
	function lihatmateri() {
		if (document.getElementById('tbupdate').style.display == "none")
			window.open("<?php echo base_url();?>virtualkelas/materi/tampilkan/<?php echo $linklist.$cekeventtahun;?>", "_self");
		else
			alert ("Silakan klik tombol Update dulu");
	}
	...
	function kosonginfile() {
		$.ajax({
			url: "<?php echo base_url();?>channel/kosonginfilemateri",
			method: "POST",
	...
	$('#submitdok').submit(function (e) {
		e.preventDefault();
		$.ajax({
			url: '<?php echo base_url(); ?>channel/upload_dok/<?php echo $linklist;?>',
			type: "post",
