...
<div class="container" style="color:black;margin-top: 60px; max-width: 900px">
	...
	<div id="addeditsoal" style="display: none">
		<div class="row">
			...
			echo form_open('vksekolah/updatesoal/'.$linklist, $attributes);
			?>
	...
	<div id="dmenubawah" style="margin-left:25px;margin-bottom: 30px;">
		<button class="btn btn-danger" onclick="return kembalivksekolah();">Kembali ke Daftar Topik</button>
...
<script>
	...
	$(document).ready(function () {
		...
		function uploadgbr(idx, field, data) {
			$('#loading' + idx).show();
			$.ajax({
				url: '<?php echo base_url(); ?>vksekolah/upload_gambarsoal/<?php echo $linklist;?>/' + pilihan + '/' + idx + '/' + field,
				type: "post",
				data: data,
				processData: false,
				contentType: false,
	...
	function ditambah() {
		...
		$.ajax({
			...
			dataType: 'json',
			cache: false,
			url: '<?php echo base_url();?>vksekolah/insertsoal/<?php echo $linklist;?>',
	...
	function dibatal() {
		...
		if (soalbaru == 1) {
			$.ajax({
				url: "<?php echo base_url();?>vksekolah/delsoal",
				method: "POST",
	...
	function cekupdate() {
		...
		teksoke2 = CKEDITOR.instances['iisiopsia'].getData().replace(/"/g, '\\x22').replace(/'/g, '\\x27');
		$('#textopsia').val(teksoke2);
		teksoke3 = CKEDITOR.instances['iisiopsib'].getData().replace(/"/g, '\\x22').replace(/'/g, '\\x27');
		$('#textopsib').val(teksoke3);
		teksoke4 = CKEDITOR.instances['iisiopsic'].getData().replace(/"/g, '\\x22').replace(/'/g, '\\x27');
		$('#textopsic').val(teksoke4);
		teksoke5 = CKEDITOR.instances['iisiopsid'].getData().replace(/"/g, '\\x22').replace(/'/g, '\\x27');
		$('#textopsid').val(teksoke5);
		teksoke6 = CKEDITOR.instances['iisiopsie'].getData().replace(/"/g, '\\x22').replace(/'/g, '\\x27');
	...
	{
		window.open("<?php echo base_url();?>channel/playlistvksekolah","_self");
	}

	function setingsoal()
	{
		window.open("<?php echo base_url();?>vksekolah/soal/seting/<?php echo $linklist;?>","_self");
	}

	function lihatsoal()
	{
		if (jmlsoal>0)
		window.open("<?php echo base_url();?>vksekolah/soal/<?php echo $linklist.'/'.$asal;?>","_self");
		else
