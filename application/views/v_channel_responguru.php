	...
	<div class="container"
		...
		opacity:90%;padding:20px;color: black;">
			...
			<div style="margin-top: 20px;">
				<?php if ($filesiswa != "") { ?>
					<button style="width:150px;padding:10px 10px;margin-bottom:5px;" class="btn-primary"
							onclick="window.open('<?php echo base_url()."channel/download/jawabansiswa/" . $linklist."/".$tugassiswa->id_user; ?>','_self')">
						Unduh Lampiran
					</button>
				<?php } ?>
			</div>
...
<script>
	...
	function updatejawaban() {
		...
		$.ajax({
			url: "<?php echo base_url();?>channel/updatetugasnilai",
			method: "POST",
	...
	function kosonginfile() {
		$.ajax({
			url: "<?php echo base_url();?>vksekolah/kosonginfilejawaban",
			method: "POST",
...
<script>
	function kembali() {
		window.open("<?php echo base_url() . 'channel/tugas/' . $npsn . '/tampilkan/' . $linklist;?>","_self");
	}
