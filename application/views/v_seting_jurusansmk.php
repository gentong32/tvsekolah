...
foreach ($dafjurusan as $datane) {
    $jml_jurusan++;
    $id[$jml_jurusan] = $datane->id;
	$jurusan[$jml_jurusan] = $datane->nama_jurusan;
}
?>
...
<script>
	...
	function ditambahsoal() {
		var jurusan = $('#iteksnilai').val();

		if (jurusan.length > 3) {
			$.ajax({
				url: "<?php echo base_url();?>seting/tambahjurusansmk",
				method: "POST",
				data: {jurusan: jurusan},
				success: function (result) {
					window.open('<?php echo base_url();?>seting/jurusansmk', '_self');
				}
			})
		}
		else {
	...
	function diedit(idx) {
        ...
        var id = new Array();
		<?php
		for ($a = 1; $a <= $jml_jurusan; $a++) {
		    echo "id[" . $a . "]='" . $id[$a] . "';";
            echo "isi[" . $a . "]='" . $jurusan[$a] . "';";
		}
		?>
	...
	function dieditsoal() {
		//alert (indeks);
		var ide = $('#iteksid').val();
        var jurusane = $('#iteksnilai').val();

        // alert (ide);
        // alert (jurusane);

		$.ajax({
			url: "<?php echo base_url();?>seting/editjurusansmk",
			method: "POST",
			data: {id: ide, jurusan: jurusane},
			success: function (result) {
				window.open('<?php echo base_url();?>seting/jurusansmk', '_self');
			}
		})
    ...
    function dihapus(idx) {
        ...
        var r = confirm("Yakin mau hapus jurusan ini?");

        if (r == true) {

            $.ajax({
                url: "<?php echo base_url();?>seting/deljurusansmk",
                method: "POST",
                data: {id: id[idx]},
                success: function (result) {
                    window.open('<?php echo base_url();?>seting/jurusansmk', '_self');
