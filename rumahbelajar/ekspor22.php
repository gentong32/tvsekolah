...
$mo_basedir = "../file_storage/modul_online";
//echo $mp_basedir;
// ---------------------------------------------------------------------

if (empty($idMateri)) exit;

$mo_dir = "$mo_basedir/mol$idMateri";
if (!is_dir($mo_dir)) 
{
mkdir($mo_dir,0755);
//exit;
...
while (false !== $file = readdir($dh)) {
  ...
  $ekstensi=explode(".",$file);
    ...
    copy("$mo_dir/$file", "$_files_dir/$file");
  if (is_dir("$mo_dir/$file"))
  { 
  $dh2 = opendir("$mo_dir/$file");
  while (false !== $file2 = readdir($dh2)) {
	$ekstensi=explode(".",$file2);
  	if ($ekstensi[1]=="html") continue; 
    if (in_array($file2, array('.','..'))) continue;
    copy("$mo_dir/$file/$file2", "$_files_dir/$file2");
  }
  closedir($dh2);
  }
}
...
{
$html_content2 = $isizipx[$a];
$html_document2 = "$zip_dir/molfiles/konten".$a.".html";
...
// open archive 
if ($zip->open("$zip_dir/mol".$idMateri."_files.zip", ZIPARCHIVE::CREATE) !== TRUE) {
...
while (false !== $file = readdir($dh)) {
  ...
  $fileList[] = array('full_path' => "$_files_dir/$file", 'local_path' => "molfiles/$file");
}
closedir($dh);
...
define('PATH', $mo_basedir.'/mol'.$idMateri."/zip/molfiles/");
destroy(PATH);
rmdir($mo_basedir.'/mol'.$idMateri."/zip/molfiles/");
unlink($mo_basedir.'/mol'.$idMateri."/zip/index.html");
