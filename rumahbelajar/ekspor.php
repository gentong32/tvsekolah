...
$mp_dir = "$mp_basedir/MP_$idMateri";
if (!is_dir($mp_dir)) 
{
mkdir($mp_dir, 0755);
//exit;
...
$html_document2 = "$zip_dir/MP_files/konten".$a.".html";
...
// open archive 
if ($zip->open("$zip_dir/MP_".$idMateri."_files.zip", ZIPARCHIVE::CREATE) !== TRUE) {
...
while (false !== $file = readdir($dh)) {
  ...
  $fileList[] = array('full_path' => "$_files_dir/$file", 'local_path' => "MP_files/$file");
}
closedir($dh);
...
define('PATH', $mp_basedir.'/MP_'.$idMateri."/zip/MP_files/");
destroy(PATH);
rmdir($mp_basedir.'/MP_'.$idMateri."/zip/MP_files/");
unlink($mp_basedir.'/MP_'.$idMateri."/zip/index.html");
