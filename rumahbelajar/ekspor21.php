...
$mp_dir = "$mp_basedir/mapok$idMateri";
if (!is_dir($mp_dir)) 
{
mkdir($mp_dir, 0755);
//exit;
...
$html_document2 = "$zip_dir/mapokfiles/konten".$a.".html";
...
// open archive 
if ($zip->open("$zip_dir/mapok".$idMateri."_files.zip", ZIPARCHIVE::CREATE) !== TRUE) {
...
while (false !== $file = readdir($dh)) {
  ...
  $fileList[] = array('full_path' => "$_files_dir/$file", 'local_path' => "mapokfiles/$file");
}
closedir($dh);
...
define('PATH', $mp_basedir.'/mapok'.$idMateri."/zip/mapokfiles/");
destroy(PATH);
rmdir($mp_basedir.'/mapok'.$idMateri."/zip/mapokfiles/");
unlink($mp_basedir.'/mapok'.$idMateri."/zip/index.html");
