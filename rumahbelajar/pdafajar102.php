...
WHERE a.status='Y' AND IDkelas>=10 AND IDkelas<=12    
UNION 
...
WHERE xa.setatus=3 AND xb.IDkelas>=10 AND xb.IDkelas<=12 
UNION
...
WHERE ya.status='Y' AND ya.IDkelas>=10 AND ya.IDkelas<=12   
UNION 
...
WHERE za.setatus=3 AND zb.IDkelas>=10 AND zb.IDkelas<=12 
...
for ($i=0;$i<$jmlmateri;$i++)
{
	...
	$idmapsinopsisel[$i]=substr(strip_tags($rows[$i]['sinopsis']),0,300);
...
<div id="topsection3">Topik Terbaru SMA</div>
...
<div id="mapelgambar" class="warnajudulgambar1"><div class="infomapel"><?php echo $namamapel[$urut[$i]]?></div><div class="infokelas">Kelas <?php echo $idkelas[$urut[$i]]?></div></div>
</a>
</div>
</div>
</div>
