...
$mo_dir = "$mo_basedir/MO_$idMateri";
if (!is_dir($mo_dir)) 
{
mkdir($mo_dir, 0755);
//exit;
...
$html_document2 = "$zip_dir/MO_files/konten".$a.".html";
...
// open archive 
if ($zip->open("$zip_dir/MO_".$idMateri."_files.zip", ZIPARCHIVE::CREATE) !== TRUE) {
...
while (false !== $file = readdir($dh)) {
  ...
  $fileList[] = array('full_path' => "$_files_dir/$file", 'local_path' => "MO_files/$file");
}
closedir($dh);
...
define('PATH', $mo_basedir.'/MO_'.$idMateri."/zip/MO_files/");
destroy(PATH);
rmdir($mo_basedir.'/MO_'.$idMateri."/zip/MO_files/");
unlink($mo_basedir.'/MO_'.$idMateri."/zip/index.html");
